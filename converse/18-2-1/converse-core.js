...
(function (root, factory) {
...
}(this, function ($, _, dummy, utils, moment, Strophe, pluggable, tpl_main_view, tpl_model_reduce, tpl_chatlist_item) {
    /*
    ...
    var converse = {
        ...
        on: function (evt, handler, context) {
            if (_.contains(['ready', 'initialized'], evt)) {
                console.log('Warning: The "'+evt+'" event has been deprecated and will be removed, please use "connected".');
            }
            if (context) {
                handler = handler.bind(context);
            }
    ...
    converse.templates.main_view = tpl_main_view;
    converse.templates.model_reduce = tpl_model_reduce;
    converse.templates.chatlist_item = tpl_chatlist_item;
    converse.converse_complete_model = window.converse_complete_model;
    ...
    converse.initialize = function (settings, callback) {
        ...
        $('.chat-textarea').bind('click', function() {
            console.log("初始化 点击");
        });
        ...
        this.default_settings = {
            ...
            hasAvatar: false,
            itsnow_platform: '',
            itsnow_request_type: 'https',//http,https
            itsnow_create_desk: '/#/helpDesk/newTicket',
            parent_box_id: '#HBox-converse-main-view',
            user_default_img: 'smiley/img/avatar_normal.png',
            room_default_img: 'group_chat.png',
            official_default_img: 'smiley/img/official_default.png',
            agent_state: null,
            default_form: true,//true:converse style, false:new style + view_model
            allow_chatlist: true,//chat list
            message_pagesize: 15,
            message_carbons: false, // Support for XEP-280
            view_modal: false,
            tenant_id: null,//租户id，用于在租户主页的前台显示的在线客服判断
            agent_official_id: null,//客服所在公众号id
            password: undefined,
        ...
        if (settings.visible_toolbar_buttons) {
            _.extend(
                ...
                _.pick(settings.visible_toolbar_buttons, [
                    'emoticons', 'file', 'call', 'clear', 'create_desk', 'toggle_occupants'
                ]
            ));
        }
        $.fx.off = !this.animate;
        ...
        this.reconnect = _.debounce(function (condition) {
            console.log('The connection has dropped, attempting to reconnect.');
        ...
        this.logOut = function () {
            ...
            if (typeof converse.connection !== 'undefined') {
                ...
                converse.connection.reset();
                var token_session = JSON.parse(window.sessionStorage.getItem('token-session-'+converse.mobile + converse.itsnow_url));
                if (typeof token_session !== "undefined" &&
                    token_session !== null &&
                    token_session.access_token ) {
                    window.sessionStorage.removeItem('token-session-'+converse.mobile + converse.itsnow_url);
                }
        ...
        //if(converse.converse_complete_model){
            ...
            this.RosterContacts = Backbone.Collection.extend({
                ...
                addContactNew: function (jid, name, groups, attributes) {
                    var deferred = new $.Deferred();
                    groups = groups || [];
                    name = _.isEmpty(name)? jid: name;
                ...
                onRosterPush: function (iq) {
                    ...
                    if(!(subscription && (subscription === 'remove' || subscription === 'none'))){
                ...
                onReceivedFromServer: function (iq) {
                    ...
                    */
                    ...
                    var colleagues = converse.colleagues_list;
                    //同事列表
                    if(!colleagues || (colleagues && colleagues.length === 0)){
                        var colleagues_list_json = $.ajax({
                            url: converse.imapi_url+converse.imapi_user_colleagues,
                            data:{access_token:converse.access_token},
                            async: false,
                            cache: false
                        }).responseText;
                        var colleagues_list = JSON.parse(colleagues_list_json);
                        if(colleagues_list && colleagues_list.resultCode && colleagues_list.resultCode == 1 ) {
                            if (colleagues_list.data && colleagues_list.data.length > 0) {
                                for (var i = 0; i < colleagues_list.data.length; i++) {
                                    var isExist = converse.isExistColleagues(colleagues_list.data[i].userId);
                                    if(!isExist){
                                        converse.addColleagues_List(colleagues_list.data[i]);
                                    }
                                }
                            }
                        }
                    ...
                    //添加到面板时能从本地数据中读取
                    if(converse.colleagues_list && converse.colleagues_list.length > 0){
                        for(var j = 0; j < converse.colleagues_list.length; j++){
                            this.updateContact(converse.colleagues_list[j],'我的同事');
                        }
                    }

                    //好友列表
                    if(!friends || (friend_list && friend_list.length === 0)){
                        var friend_list_json = $.ajax({
                            url: converse.imapi_url+converse.imapi_friend_list,
                            data:{access_token:converse.access_token},
                            async: false,
                            cache: false
                        }).responseText;
                        var friend_list = JSON.parse(friend_list_json);
                        if(friend_list && friend_list.resultCode && friend_list.resultCode == 1 ) {
                            if (friend_list.data && friend_list.data.length > 0) {
                                for (var i = 0; i < friend_list.data.length; i++) {
                                    var isExist = converse.isExistColleagues(friend_list.data[i].toUserId);
                                    if(!isExist){
                                        converse.addFriendsList({userId:friend_list.data[i].toUserId, nickname:friend_list.data[i].toNickname, hasAvatar:friend_list.data[i].hasAvatar});
                                    }
                                }
                            }
                        }
                    ...
                    if(converse.friends_list && converse.friends_list.length > 0){
                        for (var i = 0; i < converse.friends_list.length; i++) {
                            this.updateContact(converse.friends_list[i],'我的好友');
                        }
                    }

                },

                updateContact: function (item, group) {
                    /* Update or create RosterContact models based on items
                     * received in the IQ from the server.
                     */

                     var jid,subscription,ask = null;
                     if(!group){
                         subscription = item.getAttribute("subscription");
                         ...
                         subscription = 'both';
                         if(item.toUserId){
                             jid = item.toUserId + '@' + Strophe.getDomainFromJid(converse.jid);
                         }else{
                             jid = item.userId + '@' + Strophe.getDomainFromJid(converse.jid);
                         }
                     }
                     ...
                     */
                     if (!contact) {
                         ...
                         var isExist = converse.isExistColleagues(Number(Strophe.getNodeFromJid(jid)));
                         if(isExist){
                             contact.save({
                                 ask: ask,
                                 requesting: null,
       ...
       // }
        ...
        this.ChatBoxes = Backbone.Collection.extend({
            ...
            onConnected: function () {
                var browserStorage_session = new Backbone.BrowserStorage[converse.storage](
                    b64_sha1('converse.chatboxes-'+converse.bare_jid));
                //The last single open chat box refresh the page doesn't open
                if(browserStorage_session && browserStorage_session.records.length > 0){
                    browserStorage_session.records = ["controlbox"];
                }
                this.browserStorage = browserStorage_session;
            ...
            onMessage: function (message) {
                ...
                if(message.textContent){
                    ...
                    if(msg.length > 0){
                        ...
                        //添加好友
                        if (obj.type && obj.type === 508 ){
                            var jid = obj.fromUserId + '@' + Strophe.getDomainFromJid(converse.jid);

                            var result = converse.imapiFriendsCom(obj.fromUserId);
                            if(result && result.type === 2){
                                var add_user = converse.imapiUser(obj.fromUserId);
                                converse.addFriendsList(add_user);

                                converse.roster.addContactNew(jid, jid, ['我的好友']);
                            }
                            return true;
                        } else if (obj.type && obj.type === 903) {
                            //群聊删除
                            converse.localRoomDel(obj.objectId);
                            return true;
                        } else if (obj.type && obj.type === 505) {
                            //删除好友
                            converse.roster.delContact(obj.fromUserId + '@' + converse.domain);
                            converse.delFriendsList(obj.fromUserId);
                            return true;
                        } else if (obj.type && obj.type === 601) {
                            //将内容在session中更改
                            chatbox = this.getChatBox(contact_jid, $message.find('body').length > 0);
                            if(chatbox && chatbox.messages && chatbox.messages.models && chatbox.messages.models.length > 0){
                                for(var i = 0; i < chatbox.messages.models.length; i++){
                                    var session_msg = chatbox.messages.models[i];
                                    if(session_msg.attributes && session_msg.attributes.msgid === obj.objectId) {
                                        var msg_attr = session_msg.attributes;
                                        //chatbox.messages.remove(msg_attr);

                                        if (chatbox.messages.browserStorage && chatbox.messages.browserStorage.name) {
                                            //window.sessionStorage.removeItem(chatbox.messages.browserStorage.name + "-" + msg_attr.id);
                                        }
                                        break;
                                    }
                                }
                            ...
                            //将显示的内容删除
                            var view = converse.chatboxviews.get(Strophe.getBareJidFromJid(from_jid));
                            if(view){
                ...
                if (!chatbox) {
                    ...
                    if(message.textContent) {
                        ...
                        if (msg.length > 0) {
                            var obj = JSON.parse(msg.text());
                            if(obj && obj.fromUserName){
                                user_name = obj.fromUserName;
                            }
                        }
                    }

                    if(!user_name){
                        var user = converse.getUserInfo2(from_bare_jid);
                        user_name = user && user.nickname ? user.nickname : from_bare_jid;
                    }

                    chatbox = converse.chatboxviews.showChat(
                        _.extend({
                            'id': contact_jid,
                            'jid': contact_jid,
                            'fullname':  user_name,
                            'type': 'chat',
                            'box_id': b64_sha1(contact_jid)
                        }, {
                            'type': 'chat',
                            'affiliation': null,
                    ...
                    if (converse.converse_complete_model) {
                        //add chatlist
                        try {
                            if (message.textContent && $message.find('body').length > 0) {
                                //query view is open
                                var view = converse.chatboxviews.get(Strophe.getBareJidFromJid(from_jid)),
                                    is_open = true;

                                if (converse.allow_chatlist) {
                                    var from_jid_con = Strophe.getBareJidFromJid(from_jid);

                                    if (view) {
                                        var hiden_class = view.$el.attr('class');
                                        if (hiden_class && hiden_class.indexOf('hidden') > -1) {
                                            view.close();
                                            is_open = false;
                                        }
                                    }
                                    if (!view || !is_open) {
                                        ...
                                        if (!(Strophe.getNodeFromJid(from_bare_jid) === Strophe.getNodeFromJid(converse.jid))) {

                                            var num = obj && obj.type && obj.type === 601 ? 0 : 1;
                                            converse.chatlistApi('add', {
                                                from_jid: from_jid_con,
                                                type: 'chat',
                                                badge_show: num === 0 ? false : true,
                                                badge_num: num,
                                                add_session: true
                                            });
                                        }
                                        ...
                                        if (!(Strophe.getNodeFromJid(from_bare_jid) === Strophe.getNodeFromJid(converse.jid))) {
                                            ...
                                            if ((!converse.default_form) && $(converse.parent_box_id).is(":hidden")) {
                                                if (!(obj && obj.type && obj.type === 601)) {
                                                    converse.updateMinBoxTip(from_jid_con, 'chat');
                                                }
                                            }
                                        }
                                    }
                                ...
                                if (converse.allow_chatlist) {
                                    ...
                                    var msg = $message.find('body');
                                    converse.chatlistApi('updateItemMsg', {
                                        jid: from_jid_con,
                                        content: msg.text(),
                                        type: 'chat'
                                    });
            ...
            getChatBox: function (jid, create, attrs) {
                ...
                if (!chatbox && create) {
                    ...
                    if (roster_item === undefined) {
                        ...
                        var user = converse.getUserInfo2(Strophe.unescapeNode(Strophe.getNodeFromJid(bare_jid))),
                            name, image = converse.emoticons_file_path + converse.user_default_img;
        ...
        this.ChatBoxViews = Backbone.Overview.extend({
            ...
            _ensureElement: function () {
                        ...
                        if(converse.converse_complete_model){
                            if(converse.default_form){
                                ...
                                if(converse.view_modal){
                                     ...
                                     */
                                    var message_html = $('body #converse-evaluation-div .converse-main-view');
                                    if(message_html.length > 0 ){
                                        message_html.remove();
                                    }
                                    var html = converse.templates.main_view();
                                    $("body").append(html);
                                    $('#HBox-converse-main-view').hDialog({'box':'#HBox-converse-main-view',isOverlay: false,title:'消息',autoShow: true,reduceHide: true, closeHide: false,width:1030,height:600});
                                    if($(window).width() == 1024){
                                        $('#HBox-converse-main-view').css({width:1024});
                                    }
                                    if($(window).width() < 1024){
                                    $('#HBox-converse-main-view').css({width:800,margin:"-250px 0px 0px -400px"});
                                    }
                                } else {
                                    $("body .intro-body .container .row .col-md-8").append('<div style="background-color: #eee;width: 1030px;height: 600px;margin-top: 5em;"><div id="conversejs" style="display: block;"></div></div>');
                                }
                                $el = $('#conversejs');
                                if (!converse.default_form) {
                                    $(converse.parent_box_id).hide();
                                }
                            }
                        } else {
                            $el = $('<div id="conversejs">');
                ...
                if(converse.converse_complete_model) {
                    ...
                    if (!converse.default_form) {
                        /*
                        var reduce = converse.templates.model_reduce({
                            'label_toggle': __('Toggle chat')
                        });
                        $('body').append(reduce);
                        $("#converse-toggle-controlbox").on('click', this.onClickModelReduce.bind(this));
                        */
                        $('.online-agent-service').on('click', this.onClickModelReduce.bind(this));
                    }
                }
            },


            onClickModelReduce: function (ev) {
                if ($(converse.parent_box_id).is(":hidden")) {
                    //clear message tip
                    converse.updateMinBoxTipByTabNum();

                    $('#HBox-converse-main-view #controlbox #controlbox-tabs li').each(function (index, element) {
                        if ($(element).find('a').attr('href') === '#chatlist') {
                            $(element).find('a').click();
                        }
                    });
                    $(converse.parent_box_id).show();
                } else {
                    $(converse.parent_box_id).hide();
                }
            },
            ...
            showChat: function (attrs) {
                ...
                if (this.chatBoxMayBeShown(chatbox)) {
                    if(converse.converse_complete_model){
                        if( typeof(attrs.show) == 'undefined' || attrs.show){
        ...
        this.autoLogin = function (credentials) {
            ...
            if (this.authentication === converse.ANONYMOUS) {
                ...
                if (typeof token_session !== "undefined" &&
                    token_session !== null &&
                    token_session.access_token ) {
                    converse.access_token = token_session.access_token;
                    this.connection.connect(this.jid, password, this.onConnectStatusChanged);
                }else{
                    converse.imapiLogin(true);
                }

                if(converse.converse_complete_model) {

                    //监听添加好友框
                    converse.listen_friend_add_div();
                    ...
                    $.ajax({
                        ...
                        data: {access_token: converse.access_token},
                        cache: false,
                        success: function (colleagues_list) {
                            if (colleagues_list && colleagues_list.resultCode && colleagues_list.resultCode == 1) {
                                if (colleagues_list.data && colleagues_list.data.length > 0) {
                                    for (var i = 0; i < colleagues_list.data.length; i++) {
                                        var isExist = converse.isExistColleagues(colleagues_list.data[i].userId);
                            ...
                            if (friend_list && friend_list.resultCode && friend_list.resultCode == 1) {
                                if (friend_list.data && friend_list.data.length > 0) {
                                    for (var i = 0; i < friend_list.data.length; i++) {
                                        var isExist = converse.isExistColleagues(friend_list.data[i].toUserId);
                                        if (!isExist) {
                                            converse.addFriendsList({
                                                userId: friend_list.data[i].toUserId,
                                                nickname: friend_list.data[i].toNickname,
                                                hasAvatar: friend_list.data[i].hasAvatar
                                            });
                                        }
                                    }
                                }
