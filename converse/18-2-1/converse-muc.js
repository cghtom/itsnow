...
(function (root, factory) {
    ...
    var COM1 = {
        repeat1:function(key){//限制执行频率，默认为60秒 允许执行时返回false
    ...
    converse_api.plugins.add('converse-muc', {
        ...
        initialize: function () {
            ...
            converse.sendRoomMsg = function (room_jid, msgid, content) {
                ...
                converse.invitationMsg1 = function (from_jid, room_jid, recipient) {
                    var invitation = $msg({
                        from: from_jid,
                        to: room_jid,
                        id: converse.connection.getUniqueId()
                ...
                converse.ChatRoomView = converse.ChatBoxView.extend({
                    ...
                    onChatRoomMessage: function (msg) {
                        ...
                        //if(!msg_content&&this.model.get("type")=='chatroom'){
                        //    console.error("正在输入，停止输入");
                        //    return ;
                        //}
                    ...
                    fetchArchivedMessages: function (options) {
                        ...
                        if (!converse.features.findWhere({'var': Strophe.NS.MAM})) {
                            console.log("Attempted to fetch archived messages but this user's server doesn't support XEP-0313");
                            return;
                        }
                        this.addSpinner();
                        converse_api.archive.query(_.extend(options, {'groupchat': true}),
                            ...
                            function () {
                                that.clearSpinner();
                                console.log("Error while trying to fetch archived messages", "error");
                            }
                        );
                    },
            ...
            converse.RoomsPanel = Backbone.View.extend({
                ...
                sendChatRoomMessage: function (text,from,to) {
                    var msgid = converse.connection.getUniqueId();
                    var msg = $msg({
                        to: from,
                        from: to,
                ...
                imgReader : function (item) {
                    ...
                    reader.onload = function (e) {
                        ...
                        converse.uploadMessage(formData);
                    };

                    reader.readAsDataURL(blob);
                },
                pasteImg: function ( textID ) {
                ...
                createMemberRoom: function () {
                    ...
                    $.post(converse.imapi_url + converse.imapi_room_add, {
                        ...
                        if (data && data.resultCode == 1) {
                            var msg1 = '{"packetId":"'+converse.connection.getUniqueId()+'","content":"cgh12创建了群聊","fromUserId":'+Strophe.unescapeNode(Strophe.getNodeFromJid(converse.jid))+',"fromUserName":"'+converse.nick_name+'","toUserId":"'+room_jid+'","objectId":"'+room_jid+'","timeSend":'+parseInt(moment() / 1000)+',"type":10}';
            ...
            var autoJoinRooms = function () {
                ...
                _.each(converse.auto_join_rooms, function (room) {
                    if (typeof room === 'string') {
                        ...
                        console.log('Invalid room criteria specified for "auto_join_rooms"', 'error');
                    }
                });
            ...
            _.extend(converse_api, {
                'rooms': {
                    ...
                    'open': function (jids, attrs) {
                        var _that1 = this;
                        //converse_api.rooms.pastImg(jids,"groupchat");
